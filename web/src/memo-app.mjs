...
async function apiFetch(path, options = {}) {
  ...
  try {
    ...
    // The API always returns JSON errors, but keep a useful fallback for proxy failures.
  }
  if (!response.ok) {
...
async function refreshFromRemote({ silent = false } = {}) {
  if (!state.user || !window.navigator.onLine) return
  if (!silent) updateSyncStatus('正在刷新…')

  const data = await apiFetch('/api/notes')

  state.notes = applyPendingOperations(data?.notes || [], state.pending, state.user.id)
  ...
  try {
    while (state.pending.length && window.navigator.onLine) {
      ...
      if (operation.type === 'delete') {
        try {
          await apiFetch(`/api/notes/${encodeURIComponent(operation.noteId)}`, {
            method: 'DELETE',
          })
        } catch (error) {
        ...
        const { title, content, tags, category, status, is_pinned } = operation.payload
        const data = await apiFetch(`/api/notes/${encodeURIComponent(operation.noteId)}`, {
          method: 'PUT',
          body: JSON.stringify({ title, content, tags, category, status, is_pinned }),
        })
        savedNote = normalizeNote(data.note, state.user.id)
      }
...
async function signInWithPassword(event) {
  ...
  const username = elements.authEmail.value.trim()
  ...
  setAuthMessage('正在登录…')
  try {
    window.sessionStorage.setItem(`${STORAGE_PREFIX}:login-username`, username)
...
async function bootstrap() {
  ...
  elements.authEmail.value =
    window.sessionStorage.getItem(`${STORAGE_PREFIX}:login-username`) || ''
