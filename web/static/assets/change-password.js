...
async function bootstrap() {
  try {
    ...
    accountCopy.textContent = user.must_change_password
      ? `${user.username}，这是临时密码登录，请先设置自己的新密码。`
      : `${user.username}，输入当前密码后即可更新密码。`
  } catch (error) {
    if (error.status === 401) window.location.replace('/memo/')
    else showMessage('暂时无法连接 API，请稍后刷新。', 'danger')
  }
}
...
form.addEventListener('submit', async (event) => {
  ...
  if (newPassword === currentPassword) {
    showMessage('新密码不能与当前密码相同。', 'danger')
    return
  }
  ...
  try {
    ...
    form.reset()
    showMessage('密码已更新，登录状态已安全退出。正在返回登录页…', 'success')
    window.setTimeout(() => window.location.replace('/memo/'), 1200)
  } catch (error) {
    showMessage(error.code === 'invalid_credentials' ? '当前密码不正确。' : error.message, 'danger')
  } finally {
